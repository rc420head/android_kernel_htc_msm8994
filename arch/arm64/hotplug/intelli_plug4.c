...
#define DUAL_PERSISTENCE   (2400 / DEF_SAMPLING_MS)
...
#define BUSY_PERSISTENCE   (3600 / DEF_SAMPLING_MS)
...
/*
static unsigned int max_screen_off_cores = 2;
module_param(max_screen_off_cores, uint, 0664);
*/
...
#if defined(CONFIG_ARCH_MSM8994) || defined(CONFIG_ARCH_APQ8084) || \
...
static unsigned int nr_run_thresholds_balance[] = {
	...
	(THREAD_CAPACITY * 1120 * MULT_FACTOR) / DIV_FACTOR,
	(THREAD_CAPACITY * 1500 * MULT_FACTOR) / DIV_FACTOR,
	(THREAD_CAPACITY * 2100 * MULT_FACTOR) / DIV_FACTOR,
	(THREAD_CAPACITY * 2550 * MULT_FACTOR) / DIV_FACTOR,
	(THREAD_CAPACITY * 2920 * MULT_FACTOR) / DIV_FACTOR,
	(THREAD_CAPACITY * 3450 * MULT_FACTOR) / DIV_FACTOR,
	UINT_MAX
};
...
#define CPU_NR_THRESHOLD	((THREAD_CAPACITY << 1) + (THREAD_CAPACITY / (NR_CPUS / 2)))
